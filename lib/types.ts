...
export interface AppState {
  user: User | null;
  isHydrated: boolean; // Track if user data has been loaded from localStorage
  vocabSets: VocabSet[];
  setsPagination: {
    currentPage: number;
    totalPages: number;
...
export type Action =
  ...
  | { type: 'ADMIN_USERS_LOADED'; payload: { users: AdminUser[], currentPage: number, totalPages: number } };
